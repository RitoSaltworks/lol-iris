...
namespace Iris
{
    class DX9Overlay
    {
        public static Point convertToOverlayPoint(Point screenPoint, IntPtr lolHandle)
        {
            Win32Helper.RECT lolBounds;
            Win32Helper.GetWindowRect(lolHandle, out lolBounds);

            Point convertedPoint = new Point();
            
            convertedPoint.X = (int)(((double)screenPoint.X / (double)lolBounds.Right) * 800);
            convertedPoint.Y = (int)(((double)screenPoint.Y / (double)lolBounds.Bottom) * 600);

            return convertedPoint;
        }

        public static int convertToOverlayX(int X, IntPtr lolHandle)
        {
            Win32Helper.RECT lolBounds;
            Win32Helper.GetWindowRect(lolHandle, out lolBounds);

            int convertedX = (int)(((double)X / (double)lolBounds.Right) * 800);

            return convertedX;
        }

        public static int convertToOverlayY(int Y, IntPtr lolHandle)
        {
            Win32Helper.RECT lolBounds;
            Win32Helper.GetWindowRect(lolHandle, out lolBounds);

            int convertedY = (int)(((double)Y / (double)lolBounds.Bottom) * 600);
